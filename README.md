...
```json
   {
       "webhoockUrl": "ваш URL для уведомлений в Slack",
...
## **Тестирование**
Чтобы тестировать вы должны

**--Не забудьте указать ваш webhookUrl в тесте где укзано url webhook--**

**Для ReleaseService**
```bash
     cd <<путь к ReleaseService>>
```
затем пишем
...
**Для Notification**
```bash
     cd <<путь к NotificationSenderService>>
```
затем пишем
...
Тут только на двух сервисах из за того что вся логика который необходимо для теста 
```bash
     написание тестов для проверки функциональности мониторинга релизов и отправки уведомлений и
    написание интеграционных тестов для взаимодействия с GitHub API, email и Slack 
```
реализовано в ReleaseService и NotificationSenderService
